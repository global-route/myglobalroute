...
for (const [index, record] of (evidence || []).entries()) {
  const identity = record.id || '<missing-id>';
  for (const field of ['id', 'countryId', 'pathwayId', 'field', 'claim', 'sourceAuthority', 'sourceUrl', 'jurisdiction', 'retrievedAt', 'methodology', 'confidence', 'reviewAfter']) if (!record[field]) errors.push(`evidence[${index}] ${identity} ${field} is required`);
